...
> **これは設計判断の記録（ADR）です。** 2026-04-30 時点での選定理由を残したもので、
> 現在の運用手順ではありません。当時「なぜ SvelteKit を選んだか」を振り返るための記録です。
> 実際のセットアップ手順は [FRONTEND_SETUP.md](../guides/frontend/FRONTEND_SETUP.md) を参照。
